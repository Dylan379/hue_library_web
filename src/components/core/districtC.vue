                ...
                <div class='flex' v-for="(table, index) in   (orderedSeatData || '').slice(10,)  " :key="index">
                    <div>
                        <div class='ml-4' v-for="(item, index3) in   (table || '').slice(0, 4)  " :key="index3">
                            <el-icon @click="showSeatDetail(index + 11, index3 + 1); orderSeat = !orderSeat" class='mr-1'
                                :class="item.reserveNum ? (item.reserveNum > '2' ? 'fullOrdered' : 'notFullOrdered') : ''">
                                <UserFilled />
                            </el-icon>
                        </div>
                    ...
                    <div>
                        <div class='ml-1' v-for="(  item, index4  ) in   (table || '').slice(4,)  " :key="index4">
                            <el-icon @click="showSeatDetail(index + 11, index4 + 5); orderSeat = !orderSeat" class='mr-2'
                                :class="item.reserveNum ? (item.reserveNum > '2' ? 'fullOrdered' : 'notFullOrdered') : ''">
                                <UserFilled />
                            </el-icon>
                        </div>
